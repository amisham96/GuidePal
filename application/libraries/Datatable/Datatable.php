...
class Datatable {
	...
	public function run( $query ){
		...
		foreach($this->colmap as $column => $actual_col){
			$columns[ $column ] = $actual_col;
		}
		...
		if ( isset( $this->request['columns'] ) ) {
			...
			for ( $i = 0, $ien = count($this->request['columns']); $i < $ien; $i++ ) {
				...
				if ( $str != '' && $requestColumn['searchable'] == 'true' && ! isset( $columns[ $requestColumn['data'] ] ) ) {
					...
					$str0 = $str;
				    if( isset($this->extra_columns[ $requestColumn['data'] ][3]) && $this->extra_columns[ $requestColumn['data'] ][3] ){
					    $str0 = $this->_get_from_map($this->extra_columns[ $requestColumn['data'] ][3], $str0);
					}
					
					$query_clone->group_start();
					if( isset( $this->filters[ $requestColumn['data'] ] ) ){
						$query_clone = call_user_func($this->filters[ $requestColumn['data'] ][0], $query_clone, $str0, $this);
					} else if( $requestColumn['search']['regex'] == 'false' ){
						$query_clone->like( $this->_get_default_table($columns, $requestColumn['data']), $str0);
					} else {
						$query_clone->where($this->_get_default_table($columns, $requestColumn['data']) . ' REGEXP "' . addslashes($str0) . '"');
					}
		...
		if ( isset($this->request['order']) && count($this->request['order']) ) {
			...
			for ( $i = 0, $ien = count($this->request['order']); $i < $ien; $i++ ) {
				...
				if ( $requestColumn['orderable'] == 'true' && isset( $columns[ $requestColumn['data'] ] ) ) {
					...
					$orderBy[] = $this->_get_default_table($columns, $requestColumn['data']).' '.$dir;
				}
			}
	...
	public function set_colmap($arr){
		$this->colmap = $arr;
		return $this;
	}
	...
	protected function _get_default_table($columns, $col_name){
	    if( strpos($columns[$col_name], '.') || $this->table === false ){
	        return $columns[$col_name];
	    }
	    return $this->table . '.' . $columns[$col_name];
	}
	
	public function contains($str, $keyword){
